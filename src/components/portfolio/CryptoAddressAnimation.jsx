  ...
  useEffect(() => {
    ...
    const fetchETH = async () => {
      try {
        ...
        if (txData.result?.transactions && txData.result.transactions.length > 0) {
          const tx = txData.result.transactions[0];
          setTransactions(prev => ({
            ...prev,
            ETH: {
              hash: tx.hash,
  ...
  useEffect(() => {
    ...
    const fetchLINK = async () => {
      try {
        ...
        const response = await fetch(`https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=${linkContract}&page=1&offset=10&sort=desc`);
        const data = await response.json();
  ...
  useEffect(() => {
    ...
    const fetchXRP = async () => {
      try {
        ...
        if (data.result?.ledger?.transactions && data.result.ledger.transactions.length > 0) {
          const tx = data.result.ledger.transactions[0];
          setTransactions(prev => ({
            ...prev,
            XRP: {
              hash: tx.hash,
